...
export default function CurriculumSetup() {
  ...
  const [addForm, setAddForm] = useState({ departmentId: '', version: '1.0' });
  ...
  const handleAddCurriculumSubmit = async (e) => {
    ...
    try {
      let hecCourses = [];
      try {
        const selectedDept = departments.find(d => d._id === addForm.departmentId);
        if (selectedDept?.code) {
          const hecRes = await fetch(`/api/curriculums/hec?code=${encodeURIComponent(selectedDept.code)}`);
          if (hecRes.ok) {
      ...
      if (res.ok) {
        ...
        setAddForm({ departmentId: '', version: '1.0' });
  ...
  const handleOpenPublishModal = () => {
    setPublishVersionLabel('');
    setShowPublishModal(true);
  };

  const handleConfirmPublishNewVersion = async () => {
    const trimmed = publishVersionLabel.trim();
    if (!trimmed) return;
    ...
    try {
      ...
      const res = await fetch('/api/curriculums/publish-version', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
      ...
      if (res.ok) {
        ...
        setShowEditModal(false);
        fetchCurriculums(); // Refresh list
        alert(`Version "${trimmed}" published! New batches will follow it; existing batches are unaffected.`);
      } else {
        const error = await res.json();
        alert('Error: ' + error.message);
      }
      ...
      console.error(err);
      alert('Failed to publish new curriculum version');
    } finally {
      setIsSubmitting(false);
    }
  };

  const handleViewHistory = async (curriculum) => {
    const deptId = curriculum.departmentId?._id || curriculum.departmentId;
    const anyBatchInDept = batches.find(b => (b.departmentId?._id || b.departmentId) === deptId);

    if (!anyBatchInDept) {
      alert('No batches exist yet for this department, so there\'s nothing to show history against.');
      return;
    }
    ...
    try {
      const res = await fetch(`/api/curriculums/batch/${anyBatchInDept._id}/history`);
      if (res.ok) {
        const data = await res.json();
    ...
    <div style={{ padding: '0 0 40px', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      {(() => {
        const filteredCurriculums = curriculums.filter(c => {
          const matchesSearch = (c.departmentId?.name?.toLowerCase() || '').includes(searchQuery.toLowerCase()) ||
            (c.departmentId?.code?.toLowerCase() || '').includes(searchQuery.toLowerCase());
                      ...
                      {filteredCurriculums.map(c => {
                        ...
                        return (
                          <tr key={c._id} onClick={() => setSelectedCurriculum(c)} style={{ borderBottom: '1px solid #F1F5F9', cursor: 'pointer', backgroundColor: isSelected ? '#EFF6FF' : 'transparent', transition: 'background-color 0.2s' }}>
                            ...
                            <td style={{ padding: '12px 8px', color: '#0F172A', fontWeight: 500 }}>{c.departmentId?.name} Curriculum</td>
                            <td style={{ padding: '12px 8px', color: '#64748B' }}>{c.departmentId?.name}</td>
                            <td style={{ padding: '12px 8px', color: '#64748B' }}>{c.createdAt ? new Date(c.createdAt).toLocaleDateString() : 'N/A'}</td>
                            ...
                            <td style={{ padding: '12px 8px', textAlign: 'right' }}>
                              ...
                              <button onClick={(e) => { e.stopPropagation(); handleViewHistory(c); }} title="Version History" style={{ background: 'none', border: 'none', color: '#7C3AED', cursor: 'pointer', padding: '4px' }}><Clock size={14} /></button>
                            </td>
                          </tr>
                        )
                      })}
                ...
                <div style={{ backgroundColor: '#fff', borderRadius: '16px', padding: '24px', border: '1px solid #E2E8F0' }}>
                  ...
                  <div style={{ display: 'flex', gap: '16px', marginBottom: '20px' }}>
                    ...
                    <div>
                      <h4 style={{ margin: '0 0 4px', fontSize: '16px', fontWeight: 700, color: '#0F172A' }}>
                        {selectedCurriculum.departmentId?.name} Curriculum <span style={{ padding: '2px 8px', borderRadius: '12px', fontSize: '10px', fontWeight: 600, backgroundColor: selectedCurriculum.status === 'active' ? '#D1FAE5' : '#F1F5F9', color: selectedCurriculum.status === 'active' ? '#059669' : '#64748B', verticalAlign: 'middle', marginLeft: '8px' }}>{selectedCurriculum.status.toUpperCase()}</span>
                      </h4>
                      <p style={{ margin: '0 0 2px', fontSize: '13px', color: '#64748B' }}>Department: {selectedCurriculum.departmentId?.name}</p>
                      <p style={{ margin: 0, fontSize: '13px', color: '#64748B' }}>Version: {selectedCurriculum.version}</p>
                  ...
                  <div style={{ borderTop: '1px solid #E2E8F0', paddingTop: '16px' }}>
                    ...
                    <p style={{ margin: '0 0 16px', fontSize: '13px', color: '#64748B', lineHeight: 1.5 }}>
                      Official curriculum definition for {selectedCurriculum.departmentId?.name} ensuring compliance with university academic standards and program learning outcomes.
                    </p>
          ...
          <div style={{ backgroundColor: '#fff', borderRadius: '24px', width: '100%', maxWidth: '900px', maxHeight: '85vh', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '24px', borderBottom: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#F8FAFC' }}>
              <div>
                ...
                <p style={{ margin: '4px 0 0', fontSize: '12px', color: '#64748B' }}>v{selectedCurriculum.version} • {selectedCurriculum.departmentId?.name}</p>
              </div>
      ...
      {showHistoryModal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(15,23,42,0.6)', backdropFilter: 'blur(4px)', padding: '20px' }}>
          <div style={{ backgroundColor: '#fff', borderRadius: '24px', width: '100%', maxWidth: '600px', maxHeight: '80vh', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '24px', borderBottom: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#F8FAFC' }}>
              <div>
                <h3 style={{ margin: 0, fontSize: '18px', fontWeight: 800, color: '#0F172A' }}>Curriculum Version History</h3>
                <p style={{ margin: '4px 0 0', fontSize: '12px', color: '#64748B' }}>Publishing a new version never changes past ones — batches keep whatever they were pinned to.</p>
              </div>
              <button onClick={() => setShowHistoryModal(false)} style={{ background: '#E2E8F0', border: 'none', borderRadius: '50%', width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#64748B' }}>
                <X size={16} />
              </button>
            </div>
            <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '24px' }}>
              {historyLoading ? (
                ...
                <div style={{ textAlign: 'center', padding: '40px', color: '#94A3B8' }}>No version history found.</div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {historyData.map(h => (
                    <div key={h._id} style={{ padding: '14px 16px', borderRadius: '12px', border: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: h.status === 'active' ? '#F0FDF4' : '#F8FAFC' }}>
                      <div>
                        <div style={{ fontSize: '14px', fontWeight: 700, color: '#0F172A' }}>
                          v{h.version}
                          {h.status === 'active' && <span style={{ marginLeft: '8px', padding: '2px 8px', borderRadius: '10px', fontSize: '10px', fontWeight: 700, backgroundColor: '#DCFCE7', color: '#16A34A' }}>ACTIVE</span>}
                          {h.status !== 'active' && <span style={{ marginLeft: '8px', padding: '2px 8px', borderRadius: '10px', fontSize: '10px', fontWeight: 700, backgroundColor: '#F1F5F9', color: '#64748B' }}>ARCHIVED</span>}
                        </div>
                        <div style={{ fontSize: '12px', color: '#64748B', marginTop: '2px' }}>
                          {h.courseCount} courses • {new Date(h.createdAt).toLocaleDateString()}
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            <div style={{ padding: '16px 24px', borderTop: '1px solid #E2E8F0', backgroundColor: '#F8FAFC', display: 'flex', justifyContent: 'flex-end' }}>
              <button onClick={() => setShowHistoryModal(false)} style={{ padding: '10px 20px', borderRadius: '12px', border: '1px solid #E2E8F0', backgroundColor: '#fff', color: '#0F172A', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>Close</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div style={{ backgroundColor: '#fff', borderRadius: '24px', width: '100%', maxWidth: '850px', maxHeight: '85vh', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '24px', borderBottom: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#F8FAFC' }}>
              <div>
                ...
                <p style={{ margin: '4px 0 0', fontSize: '12px', color: '#64748B' }}>v{selectedCurriculum.version} • {selectedCurriculum.departmentId?.name} Curriculum</p>
              </div>
            ...
            <div style={{ padding: '16px 24px', borderTop: '1px solid #E2E8F0', backgroundColor: '#F8FAFC', display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
              ...
              <button
                ...
                title="Creates a new curriculum version — only new batches will follow it. Existing batches are unaffected."
                style={{ padding: '10px 20px', borderRadius: '12px', border: '1px solid #2563EB', backgroundColor: '#fff', color: '#2563EB', fontSize: '13px', fontWeight: 600, cursor: isSubmitting ? 'not-allowed' : 'pointer', opacity: isSubmitting ? 0.7 : 1 }}
              >
                Publish New Version
              </button>
              <button
                ...
                disabled={isSubmitting}
                title="Edits the current version in place — applies immediately to all batches following it."
      ...
      {showPublishModal && selectedCurriculum && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(15,23,42,0.6)', backdropFilter: 'blur(4px)', padding: isMobile ? '12px' : '20px' }}>
          <div style={{ backgroundColor: '#fff', borderRadius: isMobile ? '16px' : '24px', width: '100%', maxWidth: '460px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }}>
            <div style={{ padding: isMobile ? '16px' : '24px', borderBottom: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#F8FAFC', gap: '12px' }}>
              <div style={{ minWidth: 0 }}>
                <h3 style={{ margin: 0, fontSize: isMobile ? '16px' : '18px', fontWeight: 800, color: '#0F172A' }}>Publish New Version</h3>
                <p style={{ margin: '4px 0 0', fontSize: '12px', color: '#64748B', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{selectedCurriculum.departmentId?.name} Curriculum</p>
              </div>
              <button
                onClick={() => setShowPublishModal(false)}
                disabled={isSubmitting}
                style={{ flexShrink: 0, background: '#E2E8F0', border: 'none', borderRadius: '50%', width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: isSubmitting ? 'not-allowed' : 'pointer', color: '#64748B' }}
              >
                <X size={16} />
              </button>
            </div>
            ...
            <div style={{ padding: isMobile ? '16px' : '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <p style={{ margin: 0, fontSize: '13px', color: '#475569', lineHeight: 1.6 }}>
                Enter a version label for this new curriculum (e.g. "2.0"). This will only apply to <strong>new batches</strong> created from now on — batches already running will keep following their current curriculum.
              </p>
              <div>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, color: '#0F172A', marginBottom: '6px', textTransform: 'uppercase' }}>Version Label</label>
                <input
                  type="text"
                  autoFocus={!isMobile}
                  placeholder="e.g. 2.0"
                  value={publishVersionLabel}
                  onChange={e => setPublishVersionLabel(e.target.value)}
                  onKeyDown={e => {
                    if (e.key === 'Enter' && publishVersionLabel.trim() && !isSubmitting) {
                      handleConfirmPublishNewVersion();
                    }
                  }}
                  style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #CBD5E1', fontSize: '16px', outline: 'none', boxSizing: 'border-box' }}
                />
              </div>
              {publishVersionLabel.trim() && (
                <div style={{ backgroundColor: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: '12px', padding: '12px 14px', fontSize: '12px', color: '#2563EB', lineHeight: 1.5 }}>
                  Publishing <strong>"{publishVersionLabel.trim()}"</strong> — existing batches will NOT be affected; only batches created after this point will use the new version.
                </div>
              )}
            </div>

            <div style={{ padding: isMobile ? '12px 16px' : '16px 24px', borderTop: '1px solid #E2E8F0', backgroundColor: '#F8FAFC', display: 'flex', flexDirection: isMobile ? 'column-reverse' : 'row', justifyContent: 'flex-end', gap: '12px' }}>
              <button
                onClick={() => setShowPublishModal(false)}
                disabled={isSubmitting}
                style={{ width: isMobile ? '100%' : 'auto', padding: '10px 20px', borderRadius: '12px', border: '1px solid #E2E8F0', backgroundColor: '#fff', color: '#64748B', fontSize: '13px', fontWeight: 600, cursor: isSubmitting ? 'not-allowed' : 'pointer' }}
              >
                Cancel
              </button>
              <button
                onClick={handleConfirmPublishNewVersion}
                disabled={isSubmitting || !publishVersionLabel.trim()}
                style={{ width: isMobile ? '100%' : 'auto', padding: '10px 20px', borderRadius: '12px', border: 'none', backgroundColor: '#2563EB', color: '#fff', fontSize: '13px', fontWeight: 600, cursor: (isSubmitting || !publishVersionLabel.trim()) ? 'not-allowed' : 'pointer', opacity: (isSubmitting || !publishVersionLabel.trim()) ? 0.5 : 1 }}
              >

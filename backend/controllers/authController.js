...
export const resetPassword = async (req, res) => {
  try {
    ...
    // Hash and write the new password directly via updateOne, bypassing the
    // select:false/save() reliability issue (this document was not loaded
    // with .select('+password'), so setting user.password and calling
    // save() would not reliably persist the new hash).
    ...
    // Auto-login user with a fresh session
    await sendTokenCookies(req, res, user._id);

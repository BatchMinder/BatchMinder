...
export const autoGenerateTimetable = async (req, res, next) => {
  try {
    ...
    // One Curriculum document per department — fetch the batch's department curriculum directly.
    const curriculum = await resolveCurriculumForBatch(batch);

    if (!curriculum || !curriculum.courses || curriculum.courses.length === 0) {
      return res.status(404).json({ status: 'error', message: 'No active curriculum found for this department' });
    }
